...
public class Stage1 : MonoBehaviour
{
    ...
    void pan()
    {
        stage = GameObject.Find("StageMulti");
        stage.GetComponent<StageMulti>().doPanalty();
    }
    ...
    public List<Transform> GetEx(Transform tile)
    {
        ...
        // addedTiles의 각 항목을 반복
        foreach (var entry in addedTiles)
        {
            var blockList = entry.Value;

            // 현재 blockList에 입력된 tile이 있는지 확인
            foreach (Transform t in blockList)
            {
                if (t.gameObject == tile.gameObject)
                {
                    keyToRemove = entry.Key; // 입력된 tile이 포함된 blockList의 key를 저장
                    break; // 더 이상 확인할 필요 없으므로 루프 종료
                }
            }

            if (keyToRemove != null)
            {
                // 입력된 tile을 제외한 나머지 Transform을 결과 리스트에 추가
                foreach (Transform t in blockList)
                {
                    if (t.gameObject != tile.gameObject)
                    {
                ...
                break; // 해당 리스트를 찾았으므로 외부 루프 종료
            }
        }

        // identified된 key를 addedTiles에서 제거
        if (keyToRemove != null)
        {
        ...
        {
            UnityEngine.Debug.Log("해당하는 타일을 찾지 못했습니다.");
        }
